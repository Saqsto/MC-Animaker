    ...
    def draw(self, context):
        ...
        if props.show_object_panel:
            ...
            if obj and hasattr(obj, 'mc_props'):
                ...
                col.prop(mc_props, "object_type", text="")
                if mc_props.object_type == 'BLOCK':
                    ...
                    op = row.operator("mc.keyframe_property", text="", icon='KEYINGSET'); op.property_name = "block_light_level"
                    row = col.row(align=True)
                    row.prop(mc_props, "sky_light_level", text="Sky")
                    op = row.operator("mc.keyframe_property", text="", icon='KEYINGSET'); op.property_name = "sky_light_level"
                    col.separator()
                    ...
                    op = solidify_row.operator("mc.keyframe_property", text="", icon='KEYINGSET'); op.property_name = "solidify"
                    ...
                    if props.show_advanced_options:
                        adv_box = col.box()
                        adv_box.label(text="Custom NBT Data:"); adv_box.prop(mc_props, "custom_nbt", text="")
                        adv_box.separator(); adv_box.prop(mc_props, "use_custom_commands")
                        if mc_props.use_custom_commands:
                            ...
                            row.operator("mc.add_command", icon='ADD', text=""); row.operator("mc.remove_command", icon='REMOVE', text="")
        
        row = layout.row()
        icon = 'TRIA_DOWN' if props.show_assets_panel else 'TRIA_RIGHT'
        row.prop(props, "show_assets_panel", text="Assets & Datapack", icon=icon, emboss=False)
        if props.show_assets_panel:
            ...
            split = col.split(factor=0.4); split.label(text="MC Version"); split.prop(props, "minecraft_version", text="")
            col.separator()
        ...
        row = layout.row()
        panel_label = "Export Options"
        icon = 'TRIA_DOWN' if props.show_generation_panel else 'TRIA_RIGHT'
        row.prop(props, "show_generation_panel", text=panel_label, icon=icon, emboss=False)
        if props.show_generation_panel:
            box = layout.box()
            col = box.column(align=True)
            
            row = col.row(align=True)
            row.label(text="Export as:")
            row.prop(props, "export_type", expand=True)
            col.separator()

            name_label = "Scene Name" if props.export_type == 'ANIMATION' else "Model Name"
            split = col.split(factor=0.4); split.label(text=name_label); split.prop(props, "scene_name", text="")
            ...
            if props.export_type == 'ANIMATION':
                ...
                if props.use_interpolation:
                    ...
                    split = interp_box.split(factor=0.5); split.label(text="Duration"); split.prop(props, "interpolation_duration", text="")
                    split = interp_box.split(factor=0.5); split.label(text="Start"); split.prop(props, "start_interpolation", text="")
                ...
                col.separator()
                col.prop(props, "model_export_frame")
                col.prop(props, "invert_normals_on_export")
